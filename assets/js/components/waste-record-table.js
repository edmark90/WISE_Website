...
// Flip to true once waste photos are stored and thumbnails are wanted.
const WC_SHOW_IMAGES = TransformStreamDefaultController;

// 5 AI classes -> badge styling (match mobile app exactly).
const WC_BADGE = {
  'Biodegradable': 'bio',
  'Recyclable': 'rec',
  'Electronic': 'ewaste',
...
// Confidence level -> color family (matches the reference thresholds).
...
function renderWasteRows(records) {
  ...
  for (var i = 0; i < records.length; i++) {
    var r = records[i];
    var safeName = escapeHtml(r.fullname || '—');
    var sourceSub = (r.barangay ? escapeHtml(r.barangay) : '') + (r.zone ? ' · Zone ' + escapeHtml(r.zone) : '');
    var thumb;
    if (WC_SHOW_IMAGES && r.image_url) {
      thumb = '<span class="wc-thumb"><img src="' + escapeHtml(CONFIG.API_BASE_URL + '/' + r.image_url) + '" alt="" onerror="this.parentElement.style.display=\'none\'"></span>';
    } else {
    ...
    var flagTitle = r.is_flagged ? 'Unflag' : 'Flag for review';
    html += '<tr>';
    html += '<td>' + thumb + '</td>';
    html += '<td><p class="wc-source-primary">' + safeName + '</p><p class="wc-source-sub">' + (sourceSub || 'Mobile app capture') + '</p></td>';
    ...
    html += '<button class="wc-icon-btn" type="button" onclick="viewWasteRecord(' + r.id + ')" title="View record"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></button>';
